...
class Upload extends PureComponent {
    ...
    onRefresh = async () => {
        ...
        await new Promise((res) => this.setState({sections: []}, ()=>{res()}));
    ...
    fetchData = async () => {
        ...
        if(!this.onEndReachedCalledDuringMomentum){
          ...
          await new Promise((res)=> setTimeout(()=>{console.log('delay 1000ms'); res();},1000));
          const {files} = await getFiles(date);
          this.renderList(date, files);
          this.setState({isFetching: false}, () => {this.onEndReachedCalledDuringMomentum = false;});
        }
    }
